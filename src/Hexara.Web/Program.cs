...
// حضور، تاریخچه‌ی چت و قفلِ بازی هر سه در حافظه‌اند، پس این برنامه برای دقیقاً یک
// نمونه نوشته شده. با بیش از یک نمونه بازی خراب نمی‌شود (دیتابیس با توکن نسخه
// نگهش می‌دارد) ولی بازیکن‌های روی نمونه‌های مختلف حرکت همدیگر را نمی‌بینند و بات
// نوبتِ بازیکنِ حاضر را می‌زند.
//
// یک خط در لاگ، تا این فرض جایی نوشته باشد که سرِ استقرار دیده می‌شود — نه فقط
// در ‎docs/scaling.md‎.
app.Logger.LogInformation(
    "هگزارا برای یک نمونه‌ی سرور بالا آمد (حضور و چت در حافظه). برای اجرای چندنمونه‌ای "
    + "به backplane نیاز است؛ جزئیات در docs/scaling.md.");
