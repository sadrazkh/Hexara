...
export interface LegalMoves {
  ...
  robberTargets: Hex[];
  /**
   * برای هر خانه‌ی هدف (کلید «‎q,r‎»)، صندلی‌هایی که می‌شود از آن‌ها دزدید.
   *
   * سرور حسابش می‌کند نه کلاینت: قاعده چهار شرط دارد — خودت نباشی، کارت داشته
   * باشد، هم‌تیمی‌ات نباشد و با «دزد مهربان» مصون نشده باشد. فهرستِ خالی یعنی
   * «برو ولی از کسی ندزد»، که خودش یک حالتِ معتبر است.
   */
  robberVictims: Record<string, number[]>;
  /** یال‌های جاده‌ی رایگانِ کارت جاده‌سازی — با ‎roads‎ یکی نیست. */
  freeRoads: RoadAt[];
  /**
   * برای هر جاده‌ی رایگانِ اول (کلید «‎q,r,side‎»)، جاهای تازه‌ی جاده‌ی دوم.
   *
   * جاده‌ی دوم روی وضعیتِ *بعد از* اولی سنجیده می‌شود، پس فهرست بالا برایش کم
   * است. سرور حسابش می‌کند تا قاعده‌ی اتصال جاده دو جا نوشته نشود.
   */
